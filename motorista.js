...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnRegister.onclick = () => {
    ...
    const user = { id:uid(), name, email, password, role:'motorista' };
  ...
  const me = getSession();
  if(me && me.role==='motorista'){ showDash(); }
  ...
  function renderTrips(){
    ...
    if(trips.length===0){ list.innerHTML = '<p class="small">Nenhuma viagem cadastrada pela empresa.</p>'; return; }
    trips.forEach(t=>{
  ...
  document.getElementById('tabsMain').addEventListener('click', (e)=>{
    ...
    if(key==='minhas'){ document.getElementById('tab-minhas').classList.remove('hidden'); document.getElementById('tab-simulador').classList.add('hidden'); }
    else{ document.getElementById('tab-simulador').classList.remove('hidden'); document.getElementById('tab-minhas').classList.add('hidden'); }
  });

  // Simulator
  let simRows = JSON.parse(localStorage.getItem('sim_rows')||'[]');
  function persistSim(){ localStorage.setItem('sim_rows', JSON.stringify(simRows)); }
  ...
  document.getElementById('btnAddSim').onclick = ()=>{
    ...
    simRows.push({ id:uid(), data, bruto, pct:pctVal, adiant, obs });
    persistSim(); renderSim();
    document.getElementById('simBruto').value=''; document.getElementById('simPct').value=''; document.getElementById('simAdiant').value=''; document.getElementById('simObs').value='';
  };

  function renderSim(){
    const list = document.getElementById('listSim'); list.innerHTML='';
    ...
    simRows.sort((a,b)=> a.data.localeCompare(b.data)).forEach((r,i)=>{
      ...
      totalBruto += r.bruto; totalPct += ganho; totalAdiant += r.adiant;
      const item = document.createElement('div'); item.className='item';
      item.innerHTML = `
        <div>
